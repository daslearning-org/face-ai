...
class CameraAndroid(CameraBase):
    ...
    def get_display_rotation(self):
        ...
        display_rotation = rotation_map[rotation]
    ...
    def init_camera(self):
        ...
        # get camera rotation into & set camera rotation
        camera_info = CameraInfo()
        Camera.getCameraInfo(
            self._index,
            camera_info
        )
        sensor_orientation = camera_info.orientation
        display_rotation = self.get_display_rotation()
        result = (
            sensor_orientation + display_rotation
        ) % 360
        result = (360 - result) % 360
        # debug
        print(
            f"Sensor={sensor_orientation}, "
            f"Display={display_rotation}, "
            f"Result={result}"
        )
        print("Camera index:", self._index)
